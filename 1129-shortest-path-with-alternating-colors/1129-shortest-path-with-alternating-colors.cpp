...
public:
    vector<int> shortestAlternatingPaths(int n, vector<vector<int>>& r, vector<vector<int>>& b) {
        ...
        while(!q.empty()){
            int l  = q.size();
            // for every node in q check if a shorter distance exists in an alternative manner
            ...
            for(int m =0;m<l;m++){
                ...
                // if incoming edge is red then add the nodes of red and vice versa
                if(edge == 1)
                {
                    // blue
